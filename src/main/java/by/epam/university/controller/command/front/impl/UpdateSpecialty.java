	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (validation.size() != 0) {
				...
				Specialty specialty = new Specialty(Integer.parseInt(id), name, Integer.parseInt(plan),
						Integer.parseInt(year), new TypeStudy(Integer.parseInt(idTypeStudy)),
						new Faculty(Integer.parseInt(idFaculty)));

				boolean isUpdate = service.updateSpecialtyAndSubject(specialty, convertStringArrayToInt(subjects));
